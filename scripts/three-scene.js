...
if (canvas) {
  ...
  const startThreeAtmosphere = (THREE) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      const s = i * 3;
      particlePositions[s]     = THREE.MathUtils.randFloatSpread(isMobile ? 14 : 20);
      particlePositions[s + 1] = THREE.MathUtils.randFloatSpread(isMobile ? 9 : 12);
      particlePositions[s + 2] = THREE.MathUtils.randFloat(-26, 8);
      particleSeeds.push({
    ...
    const animate = () => {
      ...
      if (!prefersReducedMotion) {
        for (let i = 0; i < particleCount; i++) {
          ...
          positions[s + 1] += Math.sin(elapsed * seed.drift + seed.offset) * 0.0009;
          positions[s]     += Math.cos(elapsed * seed.drift * 0.7 + seed.offset) * 0.0007;
        }
        particleGeometry.attributes.position.needsUpdate = true;
        particles.rotation.y = elapsed * 0.018;
        hazeGroup.rotation.z = Math.sin(elapsed * 0.12) * 0.035;
        hazeGroup.children.forEach((haze, i) => {
          haze.rotation.z += 0.00045 * (i + 1);
          haze.position.y += Math.sin(elapsed * 0.22 + i) * 0.0009;
        });
      }
  ...
  import("https://cdn.jsdelivr.net/npm/three@0.163.0/build/three.module.js")
    .then(startThreeAtmosphere, startFallbackAtmosphere);
}